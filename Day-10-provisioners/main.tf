...
resource "aws_instance" "server" {
  ...
  provisioner "remote-exec" {  #server inside this will executes
    inline = [
      "touch /home/ubuntu/file200",
      "echo 'hello from jamadar' >> /home/ubuntu/file200"
    ]
  }
   provisioner "local-exec" {  # where terraform is runnig inside the directory 
    command = "touch file500" #touch is a keyword to create new file
   
 }
 }
# resource "null_resource" "run_script" {  #used this resource for run script & custom script we can run 
...
#     inline = [
#       "echo 'hello from amaaaa' >> /home/ubuntu/file200"
#     ]
#   }
